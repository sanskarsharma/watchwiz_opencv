...
df = pandas.DataFrame(columns = ["Entry", "Exit", "Session no."])
...
for sessionindex in range(0,len(timeline)):
    eachlist = timeline[sessionindex]
    for i in range(0,len(eachlist),2):
        if sessionindex ==len(timeline)-1:
            df = df.append( {"Entry" : eachlist[i] , "Exit" : 99999 , "Session no." :sessionindex }, ignore_index= True)
        else:
            df = df.append( {"Entry" : eachlist[i], "Exit" : eachlist[i+1] , "Session no." :sessionindex }, ignore_index= True )
